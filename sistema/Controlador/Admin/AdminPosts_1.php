...
class AdminPosts extends AdminControlador
{
    ...
    public function listar(): void
    {
        ...
        echo $this->template->renderizar('posts/listar.html', [
            'total' => [
                'posts' => $post->busca(null, 'COUNT(id)', 'id')->total(),
                'postsAtivo' => $post->busca('status = :s', 's=1 COUNT(status))', 'status')->total(),
                'postsInativo' => $post->busca('status = :s', 's=0 COUNT(status)', 'status')->total(),
            ]
        ]);
    }
